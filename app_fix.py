# ================================================================
# PASTE THIS ENTIRE BLOCK at the top of app.py, replacing ALL
# the try/except CSV-loading blocks for meal_plan, room_category,
# currency, ride_type, vehicle_category, vehicle.
# Then also replace get_city_hotels_by_tier() with the version below.
# ================================================================

# ── Lookup maps (built directly from supplier_service.csv patterns) ──
...
# Keep these aliases so the rest of app.py that references the old dict names still works
...
vehicle_dict        = {}   # vehicle names come from supplier_service_name directly


def _resolve(val, mapping):
    """Safely convert a raw float/int/str ID to its human-readable label."""
    if val is None:
        return ""
    s = str(val).strip()
    ...
    try:
        ...
        return s   # already a string label (e.g. rate_type stored as "Fixed Rate")
    return mapping.get(s, "")


# ================================================================
# REPLACE get_city_hotels_by_tier() with this version.
# Key changes:
#   - uses _resolve() instead of dict.get() on raw float IDs
#   - fetches ALL unique services for the hotel (not just one row)
#     and passes them as hotel["services"] for the template
# ================================================================
...
def get_city_hotels_by_tier(city, tier="medium", max_hotels=1, exclude_hotels=None):
    ...
    for row in df_hotels.itertuples():
        ...
        # ── Fetch ALL services for this supplier+accommodation ──
        ...
        # Build a deduplicated list of service dicts
        seen_names = set()
        services_list = []
        ...
        # For the top-level summary use the first active service
        ...
        hotels.append({
            ...
            # full services list for the new template table
...
# ================================================================
# ALSO REPLACE fetch_hotel_images() with this version
# — adds "services" key to each returned hotel dict
# ================================================================
...
def fetch_hotel_images(city, tier="medium", max_hotels=1, exclude_hotels=None):
    ...
    for hotel in hotels:
        ...
        result.append({
            ...
            # top-level summary fields (backward compat)
            ...
            # full services list for table rendering
