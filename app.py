...
RATE_TYPE_MAP = {
    "1": "Fixed Rate", "2": "Per Person", "3": "Per Vehicle",
}
...
# ================================================================
# 2. JOIN TABLES ONCE FOR PERFORMANCE
# ================================================================
...
{{
  ...
  "travel_style" : <string, e.g. "Cultural", "Adventure", "Beach", "Wildlife" — extract from email or "General Sightseeing">,
  "meal_plan"    : <string — extract from email if mentioned, else "Breakfast and Dinner">,
...
- If NO destinations are mentioned → choose the best cities from this list
  based on days, budget and travel style:
  {city_sample}
- Always prefer city names from the list above.

Rules for "budget":
- Extract the total trip budget if mentioned (e.g. "USD 2500" → 2500).
...
def generate_day_plan(day_num, cities, services_by_city,
    ...
    # ── Build hotel name + meal plan for lunch/dinner instructions ──
    hotel_meal_guidance = ""
    ...
    # Pick the primary hotel name for the lunch/dinner instruction
    primary_hotel = ""
...
def build_itinerary(days, schedule, user_places, budget, adults, children, travel_style, meal_plan):
    ...
    for i, cities in enumerate(schedule, start=1):
        services_by_city, full_services_by_city, hotel_info = {}, {}, {}
        for city in cities:
        ...
        result.append({
            ...
            "ai_plan": ai_plan, "budget_tier": budget_tier,
...
def get_supplier_by_id(supplier_id):
    ...
    return {
        ...
        "supplier_code":       _safe(str(r.get("supplier_code",   ""))) if "supplier_code"   in r.index else "",
        "supplier_name":       str(r.get("supplier_name",  "")),
        "supplier_address":    str(r.get("supplier_address",""))  if pd.notna(r.get("supplier_address"))  else "",
        "supplier_email":      str(r.get("supplier_email",  ""))  if pd.notna(r.get("supplier_email"))    else "",
        "supplier_telephone":  str(r.get("supplier_telephone",""))if pd.notna(r.get("supplier_telephone"))else "",
        "supplier_mobile":     str(r.get("supplier_mobile", ""))  if pd.notna(r.get("supplier_mobile"))   else "",
        "supplier_website":    str(r.get("supplier_website",""))  if pd.notna(r.get("supplier_website"))  else "",
        "supplier_rating_id":  str(r.get("supplier_rating_id",""))if pd.notna(r.get("supplier_rating_id"))else "",
        "city_id":             str(r.get("city_id",""))           if pd.notna(r.get("city_id"))           else "",
        "latitude":            str(r.get("latitude"))  if pd.notna(r.get("latitude"))  else None,
        "longitude":           str(r.get("longitude")) if pd.notna(r.get("longitude")) else None,
        "supplier_image":      str(r.get("supplier_image",  ""))  if pd.notna(r.get("supplier_image"))    else None,
        "supplier_image1":     str(r.get("supplier_image1", ""))  if pd.notna(r.get("supplier_image1"))   else None,
    }
...
def get_city_services_by_category(city, category_name, max_suppliers=10, supplier_id=None):
    ...
    for _, row in suppliers_grouped.iterrows():
        ...
        for _, srow in svc_rows.iterrows():
            ...
            services_list.append({
                "supplier": {
                    ...
                    "city_name": city,
                    "supplier_image":  str(row.get("supplier_image",  "")) if pd.notna(row.get("supplier_image"))  else "http://localhost/tripfusion/assets/img/upload/supplier/",
                    "supplier_image1": str(row.get("supplier_image1", "")) if pd.notna(row.get("supplier_image1")) else "http://localhost/tripfusion/assets/img/upload/supplier/",
                },
                "service": {
        ...
        if services_list:
            result.append({"service_category_id": str(row.get("service_category_id", "")),
                           "services": services_list})
    return result
...
def get_city_shops(city):
    ...
    deleted_mask = suppliers_df["deleted"].astype(str) == "0" if "deleted" in suppliers_df.columns else True
    df = suppliers_df[city_mask & deleted_mask].copy()
...
def _build_services_for_json(city, category_name, max_suppliers=2, restrict_supplier_id=None):
    ...
    for _, row in supplier_groups.iterrows():
        ...
        for _, sr in svc_rows.iterrows():
            ...
            if meal_id and meal_id not in ("nan","none","null"):
                svc_obj["meal_plan_id"]    = meal_id
                svc_obj["meal_plan"]       = meal_lbl
            if room_id and room_id not in ("nan","none","null"):
                svc_obj["room_category_id"]= room_id
                svc_obj["room_category"]   = room_lbl
            if curr_id and curr_id not in ("nan","none","null"):
                svc_obj["currency_id"]     = curr_id
                svc_obj["currency"]        = curr_lbl
            if rate_raw and rate_raw not in ("nan","none","null"):
                svc_obj["rate_type_id"]    = rate_raw
                svc_obj["rate_type"]       = rate_lbl
            if ride_id and ride_id not in ("nan","none","null"):
                svc_obj["ride_type_id"]    = ride_id
                svc_obj["ride_type"]       = ride_lbl
            if vcat_id and vcat_id not in ("nan","none","null"):
                svc_obj["vehicle_category_id"] = vcat_id
                svc_obj["vehicle_category"]    = vcat_lbl
            if veh_id and veh_id not in ("nan","none","null"):
                svc_obj["vehicle_id"]      = veh_id
            if alloc_int:
                svc_obj["allocated_rooms"] = alloc_int
...
def _build_meals_for_json(city, accommodation_supplier_id=None):
    if accommodation_supplier_id:
        hotel_meals = _build_services_for_json(
            city, "Meals",
            max_suppliers=1,
...
def generate_json_response(days, schedule, user_places, budget, adults, children,
    ...
    try:
        ...
        for idx, day_cities in enumerate(schedule):
            ...
            sc_imgs = {
                "image1": _city_img_url(start_city, sc.get("city_image1")),
                "image2": _city_img_url(start_city, sc.get("city_image2")),
                "image3": _city_img_url(start_city, sc.get("city_image3")),
            }
            ec_imgs = {
                "image1": _city_img_url(end_city, ec.get("city_image1")),
                "image2": _city_img_url(end_city, ec.get("city_image2")),
                "image3": _city_img_url(end_city, ec.get("city_image3")),
            }
            ...
            if iday and iday.get("hotel_images", {}).get(start_city):
                ...
                hotels = fetch_hotel_images(
                    start_city, tier=budget_tier["tier"],
                    max_hotels=1, exclude_hotels=used_hotels[start_city]
                )
                if hotels:
                    h = hotels[0]
                    acc_supplier_id = h["supplier_id"]
                    used_hotels[start_city].add(h["name"])
                    sup_info = get_supplier_by_id(acc_supplier_id) or {}
                    svcs = h.get("services", [])
                    best_svc = next(
                        (s for s in svcs if s.get("meal_plan") in meal_priority),
            ...
            route = {
                "city": {
                    "city_id":   _safe(str(ec.get("city_id", ""))),
                    "city_name": end_city,
            ...
            if idx == 0:
                route["tour_destination_progress"] = {
                    ...
                    "driver_latitude":  "0.360000",
                    "driver_longitude": "26.000000",
                }
            ...
            details.append({
                ...
                "start_city_description": get_city_description(start_city),
                "start_city_image1":      sc_imgs["image1"],
                "start_city_image2":      sc_imgs["image2"],
                "start_city_image3":      sc_imgs["image3"],
                "start_latitude":         _coord(sc.get("latitude")),
                "start_longitude":        _coord(sc.get("longitude")),
                "end_city_name":          end_city,
                "end_city_description":   get_city_description(end_city),
                "end_city_image1":        ec_imgs["image1"],
                "end_city_image2":        ec_imgs["image2"],
                "end_city_image3":        ec_imgs["image3"],
                "end_latitude":           _coord(ec.get("latitude")),
                "end_longitude":          _coord(ec.get("longitude")),
                "route": route,
                "accommodation": accommodation,
                "day_plan": ai_plan_parsed,
                "entrances":           entrances,
                "meals":               meals,
                "rides":               rides,
                "transport":           transport,
                ...
                "driver_guides":       driver_guides,
                "additional_expenses": additional,
                "tour_day_progress": None,
            })
...
def index():
    if request.method == "POST":
        try:
            ...
            days, budget, adults, children, places, travel_style, meal_plan = extract_trip_info_groq(email_text)
            budget_tier = get_budget_tier(budget, days, adults, children)
            ...
            header_data = {
                "title": "Your Sri Lanka Journey", "subtitle": "A journey crafted for you",
            ...
            full_itinerary_data = {
                "days": days, "budget": budget, "adults": adults, "children": children,
                "travel_style": travel_style, "meal_plan": meal_plan,
                "places": places, "schedule": schedule,
            ...
            return render_template(
                "itinerary.html",
                header_data=header_data, itinerary_days=itinerary_days,
                images=hero_images, days=days, budget=budget, budget_tier=budget_tier,
                adults=adults, children=children,
            ...
            if os.path.exists(itinerary_file):
                try:
                    ...
                    if full_data.get("schedule"):
                        data = full_data
                    logger.info(f"Loaded itinerary from file: {len(itinerary_days or [])} days")
                except Exception as e:
